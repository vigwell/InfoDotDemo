...
const styles = {
  ...
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 24px', height: 52, borderBottom: `1px solid ${colors.border}`, background: `linear-gradient(90deg, ${colors.surface} 0%, #172a2e 100%)` },
  headerLeft: { display: 'flex', alignItems: 'center', gap: 12 },
  headerRight: { display: 'flex', alignItems: 'center', gap: 8 },
  logo: { fontSize: 20, fontWeight: 800, color: colors.accent, letterSpacing: '-0.5px', fontVariantNumeric: 'tabular-nums' },
  ...
  statusDot: (ready) => ({ width: 8, height: 8, borderRadius: '50%', background: ready ? colors.green : colors.amber, display: 'inline-block', boxShadow: ready ? `0 0 6px ${colors.green}` : `0 0 6px ${colors.amber}` }),
  ...
  studyModality: (mod) => ({ width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0, background: mod === 'CT' ? '#0eb8a822' : mod === 'MR' ? '#a78bfa22' : '#f59e0b22', color: mod === 'CT' ? '#0eb8a8' : mod === 'MR' ? '#a78bfa' : '#f59e0b' }),
  ...
  eventType: (type) => ({ fontSize: 10, fontWeight: 600, padding: '1px 5px', borderRadius: 3, marginRight: 6, background: type === 'text-changed' ? '#0eb8a820' : type === 'clicked' ? '#f59e0b20' : type === 'action' ? '#22c55e20' : '#a78bfa20', color: type === 'text-changed' ? '#0eb8a8' : type === 'clicked' ? '#f59e0b' : type === 'action' ? '#22c55e' : '#a78bfa' }),
  ...
  patientBanner: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderRadius: 10, background: colors.surface, border: `1px solid ${colors.border}`, borderLeft: `3px solid ${colors.accent}`, marginBottom: 16 },
  ...
  bannerStatus: (status) => ({ fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 6, letterSpacing: '0.5px', background: status === 'signed' ? '#22c55e20' : status === 'addendum' ? '#f59e0b20' : `${colors.accent}18`, color: status === 'signed' ? '#22c55e' : status === 'addendum' ? '#f59e0b' : colors.accent }),
  studyBanner: { display: 'flex', gap: 24, padding: '14px 20px', borderRadius: 10, background: colors.surface, border: `1px solid ${colors.border}`, marginBottom: 22 },
  infoField: {},
  ...
  textarea: { width: '100%', backgroundColor: '#1e2f34', border: `1px solid ${colors.border}`, borderRadius: 10, color: colors.text, WebkitTextFillColor: colors.text, forcedColorAdjust: 'none', padding: '14px 16px', fontSize: 14, fontFamily: "'SF Pro Text', -apple-system, system-ui, sans-serif", lineHeight: 1.6, resize: 'vertical', outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s' },
  actions: { display: 'flex', gap: 12, marginTop: 20 },
  btnPrimary: { padding: '10px 24px', borderRadius: 8, border: 'none', background: `linear-gradient(90deg, ${colors.accent}, #0891b2)`, color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', boxShadow: `0 2px 12px ${colors.accent}40` },
  ...
  hint: { marginTop: 24, padding: '14px 18px', borderRadius: 10, background: `${colors.accent}0e`, border: `1px solid ${colors.accent}28`, fontSize: 13, lineHeight: 1.6, color: colors.textMuted },
};
