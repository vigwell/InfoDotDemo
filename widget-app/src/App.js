...
const LockIcon = () => (
  <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#4f83ff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <rect x="5" y="11" width="14" height="10" rx="2"/>
    <path d="M8 11V7a4 4 0 0 1 8 0v4"/>
    <circle cx="12" cy="16" r="1.2" fill="#4f83ff" stroke="none"/>
  </svg>
);
...
const s = {
  shell: {
    ...
    background: 'radial-gradient(ellipse at 30% 0%,#111826 0%,#0c0f1a 60%)',
    color: '#dce6f8',
  ...
  header: {
    ...
    background: 'linear-gradient(180deg,rgba(15,24,44,0.95) 0%,rgba(12,15,26,0) 100%)',
    borderBottom: '1px solid rgba(79,131,255,0.18)',
    backdropFilter: 'blur(8px)',
  },
  logoRow: { display: 'flex', alignItems: 'center', gap: 10 },
  logoName: { fontSize: 14, fontWeight: 700, color: '#c8d8ff', letterSpacing: '-0.2px', lineHeight: 1.2 },
  logoSub: { fontSize: 9, color: '#4a5980', fontWeight: 500, letterSpacing: '0.5px', textTransform: 'uppercase', marginTop: 1 },
  headerRight: { display: 'flex', alignItems: 'center', gap: 8 },
  statusPill: { display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' },
  statusDot: (active) => ({ width: 6, height: 6, borderRadius: '50%', background: active ? '#f59e0b' : '#34d399', boxShadow: active ? '0 0 6px #f59e0b' : '0 0 6px #34d399', animation: active ? 'pulse 1s infinite' : 'none' }),
  statusLabel: { fontSize: 10, color: '#7b88a8', fontWeight: 500 },
  iconBtn: { width: 30, height: 30, borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)', color: '#7b88a8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  contextArea: { flex: 1, overflowY: 'auto', padding: '10px 12px 4px', minHeight: 0 },
  suggestionsPanel: { flexShrink: 0, background: 'rgba(10,13,22,0.95)', borderTop: '1px solid rgba(79,131,255,0.22)', boxShadow: '0 -8px 32px rgba(0,0,0,0.4)' },
  ...
  suggDot: (active) => ({ width: 6, height: 6, borderRadius: '50%', background: active ? '#f59e0b' : '#4f83ff', boxShadow: active ? '0 0 5px #f59e0b' : '0 0 5px #4f83ff', animation: active ? 'pulse 1s infinite' : 'none' }),
  suggTitle: { fontSize: 10, fontWeight: 700, color: '#7b88a8', textTransform: 'uppercase', letterSpacing: '1px' },
  suggBadge: { fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 10, background: 'rgba(79,131,255,0.15)', color: '#4f83ff', border: '1px solid rgba(79,131,255,0.2)' },
  suggList: { padding: '0 12px 10px', maxHeight: 220, overflowY: 'auto' },
  suggEmpty: { fontSize: 11, color: '#3d4a65', textAlign: 'center', padding: '10px 8px 14px', fontStyle: 'italic' },
  suggAnalyzing: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 8px 14px', justifyContent: 'center', fontSize: 11, color: '#7b88a8' },
  miniSpinner: { width: 13, height: 13, border: '2px solid rgba(255,255,255,0.08)', borderTopColor: '#4f83ff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' },
  toast: { position: 'absolute', bottom: 36, left: 12, right: 12, padding: '9px 14px', borderRadius: 9, background: 'linear-gradient(90deg,#10b981,#059669)', color: '#fff', fontSize: 12, fontWeight: 600, textAlign: 'center', animation: 'slideUp 0.3s ease', zIndex: 50, boxShadow: '0 4px 20px rgba(16,185,129,0.4)' },
  footer: { padding: '5px 14px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, flexShrink: 0, background: 'rgba(8,10,18,0.8)' },
  footerText: { fontSize: 9, color: '#2d3550', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' },
  footerSep: { width: 3, height: 3, borderRadius: '50%', background: '#1e2640', display: 'inline-block' },

  // Modal
  overlay: { position: 'absolute', inset: 0, zIndex: 100, background: 'rgba(5,8,16,0.82)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  modal: { width: 348, maxWidth: '94%', borderRadius: 16, background: 'linear-gradient(160deg,#141a2e 0%,#0f1220 100%)', border: '1px solid rgba(79,131,255,0.2)', boxShadow: '0 24px 80px rgba(0,0,0,0.7)', overflow: 'hidden', maxHeight: '88vh', display: 'flex', flexDirection: 'column' },
  modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(79,131,255,0.06)', flexShrink: 0 },
  modalHeaderLeft: { display: 'flex', alignItems: 'center', gap: 8 },
  modalTitle: { fontSize: 13, fontWeight: 700, color: '#c8d8ff' },
  modalCloseBtn: { width: 26, height: 26, borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', background: 'none', color: '#4a5980', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  backBtn: { width: 26, height: 26, borderRadius: 6, border: '1px solid rgba(255,255,255,0.08)', background: 'none', color: '#7b88a8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },

  // Lock screen
  lockScreen: { padding: '28px 22px 22px', textAlign: 'center' },
  lockIconWrap: { marginBottom: 14, display: 'flex', justifyContent: 'center' },
  lockTitle: { fontSize: 15, fontWeight: 700, color: '#c8d8ff', marginBottom: 5 },
  lockSub: { fontSize: 11, color: '#4a5980', marginBottom: 20, lineHeight: 1.6 },
  pwInput: { width: '100%', padding: '11px 14px', borderRadius: 9, border: '1px solid', background: 'rgba(255,255,255,0.04)', color: '#dce6f8', fontSize: 15, outline: 'none', boxSizing: 'border-box', textAlign: 'center', letterSpacing: '6px', marginBottom: 8, transition: 'border-color 0.2s' },
  pwError: { fontSize: 11, color: '#f87171', marginBottom: 12 },
  primaryBtn: { width: '100%', padding: '11px', borderRadius: 9, border: 'none', background: 'linear-gradient(90deg,#4f83ff,#7c5af6)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 16px rgba(79,131,255,0.35)', marginTop: 4 },
  ghostBtn: { flex: 1, padding: '10px', borderRadius: 9, border: '1px solid rgba(255,255,255,0.1)', background: 'none', color: '#7b88a8', fontSize: 12, fontWeight: 600, cursor: 'pointer' },

  // Integrations list
  listBody: { padding: '14px 16px 18px', overflowY: 'auto' },
  listIntro: { fontSize: 11, color: '#4a5980', lineHeight: 1.6, marginBottom: 14, padding: '10px 12px', background: 'rgba(79,131,255,0.06)', borderRadius: 8, border: '1px solid rgba(79,131,255,0.12)' },
  intgCard: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 13px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, marginBottom: 8 },
  intgCardLeft: { display: 'flex', alignItems: 'center', gap: 10 },
  intgDot: { width: 8, height: 8, borderRadius: '50%', flexShrink: 0 },
  intgName: { fontSize: 13, fontWeight: 700, color: '#c8d8ff' },
  intgDesc: { fontSize: 10, color: '#4a5980', marginTop: 2 },
  intgActions: { display: 'flex', alignItems: 'center', gap: 6 },
  editBtn: { padding: '4px 12px', borderRadius: 6, border: '1px solid rgba(79,131,255,0.3)', background: 'rgba(79,131,255,0.08)', color: '#4f83ff', fontSize: 11, fontWeight: 600, cursor: 'pointer' },
  deleteBtn: { width: 26, height: 26, borderRadius: 6, border: '1px solid rgba(239,68,68,0.2)', background: 'none', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.7 },
  addSystemBtn: { width: '100%', padding: '10px', borderRadius: 9, border: '1px dashed rgba(79,131,255,0.3)', background: 'rgba(79,131,255,0.04)', color: '#4f83ff', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 4 },

  // Edit form
  editBody: { padding: '14px 16px 18px', overflowY: 'auto' },
  editField: { marginBottom: 14 },
  editLabel: { display: 'block', fontSize: 10, fontWeight: 600, color: '#4a5980', textTransform: 'uppercase', letterSpacing: '0.7px', marginBottom: 6 },
  editLabelRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  editInput: { width: '100%', padding: '9px 11px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.04)', color: '#c8d8ff', fontSize: 12, outline: 'none', boxSizing: 'border-box' },
  editTextarea: { width: '100%', padding: '9px 11px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.04)', color: '#c8d8ff', fontSize: 11, outline: 'none', boxSizing: 'border-box', fontFamily: 'monospace', resize: 'vertical', lineHeight: 1.55 },
  editHint: { fontSize: 10, color: '#3d4a65', marginTop: 5 },
  code: { fontFamily: 'monospace', background: 'rgba(79,131,255,0.1)', padding: '1px 5px', borderRadius: 3, color: '#7b9cf8' },
  mappingRow: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 },
  mappingInput: { flex: 1, padding: '7px 9px', borderRadius: 7, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)', color: '#c8d8ff', fontSize: 11, outline: 'none', minWidth: 0 },
  mappingArrow: { fontSize: 10, color: '#3d4a65', flexShrink: 0 },
  removeMappingBtn: { width: 24, height: 24, borderRadius: 5, border: '1px solid rgba(239,68,68,0.2)', background: 'none', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: 0.7 },
  addMappingBtn: { display: 'flex', alignItems: 'center', padding: '4px 10px', borderRadius: 6, border: '1px solid rgba(79,131,255,0.25)', background: 'rgba(79,131,255,0.07)', color: '#4f83ff', fontSize: 10, fontWeight: 600, cursor: 'pointer' },
